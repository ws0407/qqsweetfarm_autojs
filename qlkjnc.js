...
if(!requestScreenCapture()){
 ...
 if(files.isDir("/sdcard/windows/Pictures/qqsweetfarm/")){
    log("src exists!");
    sleep(1000);
}else{
    toast("src lost!");
...
// load sources
var all_pictures = files.listDir("/sdcard/windows/Pictures/qqsweetfarm/");
var all_imgs = {};

for(var key in all_pictures){
    all_imgs[all_pictures[key]] = images.read('/sdcard/windows/Pictures/qqsweetfarm/'+all_pictures[key]);
}
toast("脚本启动成功");
...
function clickImage(path, point){
    var pixels = images.readPixels("/sdcard/windows/Pictures/qqsweetfarm/" + path);
