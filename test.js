...
if(img.getWidth() < img.getHeight()){
    ...
    sleep(1000);
    toast("App setup done");
    log("App setup done");
}
img.recycle();


toastLog(restartApp(appName));
...
var flag = true;
var times = 1;
var status = 3;     // 1:harvest, 2:sow, 3:sell, 4:wrong
var is_crashed = 0;
while(flag){
    var img = images.captureScreen();
    if(img.getWidth() < img.getHeight()){
        ...
        if(!point_wheat_sow){
            ...
            if(!point_wheat_sow){
                ...
                if(point_sickle){
                    ...
                    log("maybe sometiing goes to wrong...");
                    sleep(10000);
                }
                continue;
            }
...
function sellWheat(){
    ...
    var warehouse_wheat = findImage(_img, all_imgs["warehouse_wheat.png"], 
                        {threshold: 0.8});
    log("warehouse_wheat"+ warehouse_wheat);
    toast("warehouse_wheat"+ warehouse_wheat);
    sleep(10000);
    exit();
